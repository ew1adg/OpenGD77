	...
	else
	{
		...
		if (nLines > 1)
		{
			...
			if (rest > 21)
			{
				...
				ucPrintCentered(y, buffer, FONT_XS); // 2 pixels are saved, could center

				buffer[21] = c;
				buffer[42] = 0;

				ucPrintCentered(y + 8, buffer + 21, FONT_XS);
			}
			else
			{
				*p = 0;

				ucPrintCentered(y, buffer, FONT_XS);
				ucPrintCentered(y + 8, p + 1, FONT_XS);
			}
		}
		else // One line of 21 chars max
		{
			ucPrintCentered(y + 4, text, FONT_XS);
		}
	}
}
...
void menuUtilityRenderHeader(void)
{
	...
	switch(trxGetMode())
	{
		case RADIO_MODE_ANALOG:
			...
			ucPrintCore(0, Y_OFFSET, buffer, FONT_XS, TEXT_ALIGN_LEFT, scanBlinkPhase);
			...
			if ((currentChannelData->txTone!=65535)||(currentChannelData->rxTone!=65535))
			{
				...
				ucPrintCore(24, Y_OFFSET, buffer, FONT_XS, TEXT_ALIGN_LEFT, isInverted);
			}
			break;
...
//				(trxGetMode() == RADIO_MODE_DIGITAL && settingsPrivateCallMuteMode == true)?" MUTE":"");// The location that this was displayed is now used for the power level
				...
				if (!scanBlinkPhase)
				{
					...
					ucPrintCore(DMR_TEXT_X_OFFSET, Y_OFFSET, "DMR", ((nonVolatileSettings.hotspotType != HOTSPOT_TYPE_OFF) ? FONT_XS_BOLD : FONT_XS), TEXT_ALIGN_LEFT, isInverted);
				}
				...
				if (nonVolatileSettings.dmrFilterLevel < DMR_FILTER_CC_TS)
				{
					...
					ucPrintCore(22, Y_OFFSET, buffer, FONT_XS, TEXT_ALIGN_LEFT, true);
				}
				else
				{
					ucPrintCore(22, Y_OFFSET, buffer, FONT_XS, TEXT_ALIGN_LEFT, false);
//					if (nonVolatileSettings.tsManualOverride != 0)
//					{
//						ucFillRect(34, Y_OFFSET, 7, 8, false);
//						snprintf(buffer, bufferLen, "%d", trxGetDMRTimeSlot() + 1);
//						ucPrintCore(35, Y_OFFSET, buffer, FONT_XS, TEXT_ALIGN_LEFT, true);
//					}
				}
			}
			break;
	...
	ucPrintCentered(Y_OFFSET, (char *)POWER_LEVELS[nonVolatileSettings.txPowerLevel], FONT_XS);
	...
	if (settingsUsbMode == USB_MODE_HOTSPOT || trxGetMode() == RADIO_MODE_ANALOG)
	{
		...
		buffer[bufferLen - 1] = 0;
		ucPrintCore(0, Y_OFFSET, buffer, FONT_XS, TEXT_ALIGN_RIGHT, false);// Display battery percentage at the right
	}
	else
	{
		...
		ucPrintCore(COLOR_CODE_X_POSITION, Y_OFFSET, buffer, FONT_XS, TEXT_ALIGN_LEFT, nonVolatileSettings.dmrFilterLevel == DMR_FILTER_NONE);
